...
- Relay-sourced channels that are briefly scrambled while the receiver's descrambler locks are no longer falsely rejected with R_UPSTREAM_SCRAMBLED. The input preflight now reads past the lock window before classifying; genuinely scrambled channels are still rejected.
