...
if [ "${#scan_targets[@]}" -gt 0 ]; then
  if command -v rg >/dev/null 2>&1; then
    if rg -n -- "$PATTERN" "${scan_targets[@]}"; then
      ...
      echo "   Use imports from src/client-ts (index) or src/client-ts/wrapper."
      exit 1
    fi
  else
  ...
  if command -v rg >/dev/null 2>&1; then
    if rg -n --glob '*.{ts,tsx,js,jsx}' --glob '!src/client-ts/**' -- "$PATTERN" src tests; then
      echo "❌ Direct client-ts/*.gen import detected outside wrapper boundary."
      echo "   Use imports from src/client-ts (index) or src/client-ts/wrapper."
      exit 1
    fi
