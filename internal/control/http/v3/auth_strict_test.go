...
func TestLANGuard_UntrustedProxy(t *testing.T) {
	...
	t.Run("chained_proxy_right_to_left", func(t *testing.T) {
		...
		if resp.StatusCode != http.StatusForbidden {
			...
			t.Errorf("expected 403 (8.8.8.8 resolved as client), got %d. Body: %s", resp.StatusCode, string(body))
		}
	})
}
...
func newTestServerConfig(t *testing.T, spy *SpyStore, spyBus *SpyBus, fn func(*config.AppConfig)) (testServer, testTokens) {
	...
	srv.SetDependencies(b, spy, resume.NewMemoryStore(), nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
	srv.SetPreflightCheck(nil)

	// Initialize LAN Guard from config
	lg, err := middleware.NewLANGuard(middleware.LANGuardConfig{
		TrustedProxyCIDRs: strings.Split(cfg.TrustedProxies, ","),
	})
	if err != nil {
