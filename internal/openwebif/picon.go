...
func PiconURL(owiBase, sref string) string {
	...
	// We do NOT normalize by default, creating the URL for the EXACT reference passed.
	// Normalization (HD->SD fallback) should be handled by the caller (http.go) if the exact fetch fails.
