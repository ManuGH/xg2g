...
func BuildHLSArgs(in InputSpec, out OutputSpec, prof model.ProfileSpec) ([]string, error) {
	...
	args := []string{
		...
		"-fflags", "+genpts+discardcorrupt+igndts+flush_packets", // Generate PTS, discard corrupt, ignore DTS, flush on segments
	...
	if prof.VOD {
		...
		// LIVE-DVR: Keep all segments (playlist grows)
		playlistSize = 0
	...
	// LIVE-DVR (always): EVENT playlist, segments kept until session ends
	...
	if prof.VOD {
		...
		// LIVE-DVR mode: EVENT playlist, NO delete_segments (Timeline/CHASE requires all segments)
		// Segments remain until session cleanup or explicit retention policy.
		hlsFlags = "append_list+omit_endlist+independent_segments+program_date_time+temp_file"
		playlistType = "event"
		...
		log.L().Debug().
			...
			Str("playlist_type", playlistType).
			Msg("live-dvr mode ffmpeg configuration (segments kept)")
	} else if prof.LLHLS {
	...
	if prof.TranscodeVideo {
		...
		if prof.VideoCodec == "hevc" {
			if prof.HWAccel != "vaapi" {
				...
				// Open-GOP disabled for HLS usually? x265 defaults to open-gop.
				// HLS prefers closed gop. keyint=min-keyint implies fixed GOP, but open-gop might still be active.
