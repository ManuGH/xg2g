...
// Private helper copy for testing (since tests share package api, we can just call it if we export it,
...
// I made it private `escapeServiceRefPath`. Since I am in `package api` (same package), I can access it.
// Wait, test file is `package api` or `package api_test`?
// I declared `package api` in the previous write. So I can call `escapeServiceRefPath`.
