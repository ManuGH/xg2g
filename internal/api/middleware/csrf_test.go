...
func TestGetRequestOrigin_ReturnsEmptyForRelativeReferer(t *testing.T) {
	req := httptest.NewRequest(http.MethodPost, "/test", nil)
	req.Header.Set("Referer", "/relative/path")

	origin := getRequestOrigin(req)
	if origin != "" {
		t.Errorf("Expected empty string for relative referer, got %s", origin)
	}
}

func TestGetRequestOrigin_ReturnsEmptyForUnsupportedRefererScheme(t *testing.T) {
	req := httptest.NewRequest(http.MethodPost, "/test", nil)
	req.Header.Set("Referer", "file:///tmp/index.html")

	origin := getRequestOrigin(req)
	if origin != "" {
		t.Errorf("Expected empty string for unsupported referer scheme, got %s", origin)
	}
}

func TestGetRequestOrigin_NormalizesDefaultHTTPSPort(t *testing.T) {
	req := httptest.NewRequest(http.MethodPost, "/test", nil)
...
func TestCSRFProtection_AllowsConfiguredOriginsWithDefaultPortNormalization(t *testing.T) {
	os.Setenv("XG2G_ALLOWED_ORIGINS", "https://trusted.com:443") //nolint:errcheck // Test setup
	defer os.Unsetenv("XG2G_ALLOWED_ORIGINS")                    //nolint:errcheck // Test cleanup
	...
	if w.Code != http.StatusOK {
		t.Errorf("Allowed origin with default port normalization: expected 200, got %d", w.Code)
	}
}
