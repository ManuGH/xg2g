...
func TestShouldTrace(t *testing.T) {
	...
	tracePaths := []string{"/api/status", "/v1/streams", "/files/playlist.m3u"}
...
func TestSpanNameFormatter(t *testing.T) {
	...
	if got := spanNameFormatter("HTTP GET", req); got != "HTTP GET /api/status" {
		t.Fatalf("unexpected span name: %s", got)
	}

	reqWithQuery := httptest.NewRequest(http.MethodGet, "/api/status?token=secret", nil)
	if got := spanNameFormatter("HTTP GET", reqWithQuery); got != "HTTP GET /api/status?" {
