...
func readStableFile(path string, maxWait time.Duration) ([]byte, error) {
	...
	for {
		...
		if i1.ModTime().Equal(i2.ModTime()) && i1.Size() == i2.Size() && bytes.Equal(b1, b2) {
			return b2, nil
		}
...
func (p *SafariDVRProfile) waitForSegments(playlistPath string, ready chan struct{}) {
	...
	for {
		...
		case <-ticker.C:
			...
			data, err := readStableFile(playlistPath, 250*time.Millisecond)
...
func (p *SafariDVRProfile) ServePlaylist(w http.ResponseWriter) error {
	...
	playlistPath := filepath.Join(p.outputDir, "playlist.m3u8")
	data, err := readStableFile(playlistPath, 500*time.Millisecond)
	if err != nil {
