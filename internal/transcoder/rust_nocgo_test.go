...
// TestRustAudioRemuxer_Stub verifies that the stub returns an appropriate error
func TestRustAudioRemuxer_Stub(t *testing.T) {
	_, err := NewRustAudioRemuxer(48000, 2, 192000)
	if err == nil {
		t.Fatal("NewRustAudioRemuxer should fail when CGO is disabled or transcoder tag missing")
	}
	...
	// Test stub struct directly
	remuxerStr := &RustAudioRemuxer{}
	output := make([]byte, 1024)
	written, err := remuxerStr.Process([]byte("test"), output)
	if err == nil {
		t.Error("Process should return error")
	}
	if written != 0 {
		t.Error("Process should return 0 bytes written")
	}

	if err := remuxerStr.Close(); err != nil {
...
func TestRustAudioRemuxer_Process_Stub(t *testing.T) {
	...
	output := make([]byte, 1024)
	written, err := remuxer.Process([]byte{0x47, 0x00}, output)
	if err == nil {
