...
ffmpeg_pids=$(pgrep -f "ffmpeg.*stream.m3u8" || true)
...
for pid in $ffmpeg_pids; do
    ...
    if echo "$cmdline" | grep -q -- "-hls_time"; then
        ...
        if [[ "$hls_time" == "6" ]]; then
            echo "   ✅ hls_time: $hls_time (Best Practice 2026)"
        else
            echo "   ⚠️  hls_time: $hls_time (Expected: 6)"
        fi
    else
    ...
    if echo "$cmdline" | grep -q -- "-hls_list_size"; then
        ...
        if [[ "$hls_list_size" == "10" ]]; then
            window_sec=$((hls_list_size * 6))
            echo "   ✅ hls_list_size: $hls_list_size (${window_sec}s window - Best Practice 2026)"
        else
            echo "   ⚠️  hls_list_size: $hls_list_size (Expected: 10)"
        fi
    else
    ...
    session_dir=$(echo "$cmdline" | grep -oP '/[^ ]+/stream\.m3u8' | xargs dirname || echo "unknown")
    echo "   $session_dir"

    if [[ -d "$session_dir" ]]; then
        segment_count=$(find "$session_dir" -name "stream*.ts" 2>/dev/null | wc -l)
        echo "   Segments: $segment_count"

        if [[ -f "$session_dir/stream.m3u8" ]]; then
            playlist_segments=$(grep -c "stream[0-9]*.ts" "$session_dir/stream.m3u8" || echo "0")
            echo "   Playlist entries: $playlist_segments"

            if [[ "$playlist_segments" -gt 0 ]]; then
                first_segment=$(grep -oE "stream[0-9]+\.ts" "$session_dir/stream.m3u8" | head -1 || echo "none")
                last_segment=$(grep -oE "stream[0-9]+\.ts" "$session_dir/stream.m3u8" | tail -1 || echo "none")
...
echo ""
echo "💡 Summary:"
echo "   Expected configuration (Best Practice 2026):"
