...
REF_WIDTH=$(ffprobe -v error -select_streams v:0 -show_entries stream=width -of default=nokey=1:raw_value=1 "$DECODED_REF")
REF_HEIGHT=$(ffprobe -v error -select_streams v:0 -show_entries stream=height -of default=nokey=1:raw_value=1 "$DECODED_REF")
...
for TARGET_K in "${BITRATES_K[@]}"; do
    ...
    TEST_FRAMES=$(ffprobe -v error -count_frames -select_streams v:0 -show_entries stream=nb_read_frames -of default=nokey=1:raw_value=1 "$TEST_OUT")
    TEST_WIDTH=$(ffprobe -v error -select_streams v:0 -show_entries stream=width -of default=nokey=1:raw_value=1 "$TEST_OUT")
    TEST_HEIGHT=$(ffprobe -v error -select_streams v:0 -show_entries stream=height -of default=nokey=1:raw_value=1 "$TEST_OUT")
