...
# Version information
# The embedded webui dist is committed but regenerated by local UI builds, so
# `git describe --dirty` would stamp every build after `make build-with-ui` as
...
# from backend/api/openapi.yaml. Named here because several targets refer to them.
...
NATIVE_CONTRACT_MANIFEST := backend/api/contract.manifest.json
GO := go
