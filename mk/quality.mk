...
test-cover: ## Run tests with coverage reporting
	...
	@cd $(BACKEND_DIR) && $(RESOLVE_GO_BIN_SH) && GOTOOLCHAIN=local "$$GO_BIN" test -count=1 -timeout=$(GO_TEST_COVER_TIMEOUT) -covermode=atomic -coverprofile=../$(ARTIFACTS_DIR)/coverage.out -coverpkg=./internal/...,./test/... ./...
	@cd $(BACKEND_DIR) && $(RESOLVE_GO_BIN_SH) && GOTOOLCHAIN=local "$$GO_BIN" tool cover -html=../$(ARTIFACTS_DIR)/coverage.out -o ../$(ARTIFACTS_DIR)/coverage.html
	@echo "Coverage report generated: $(ARTIFACTS_DIR)/coverage.html"
	@cd $(BACKEND_DIR) && $(RESOLVE_GO_BIN_SH) && GOTOOLCHAIN=local "$$GO_BIN" tool cover -func=../$(ARTIFACTS_DIR)/coverage.out | tail -1
