...
func generateSampleTS(t *testing.T) []byte {
	...
	cmd := exec.Command("ffmpeg", "-y", "-f", "lavfi", "-i", "testsrc=duration=30:size=320x240:rate=30", "-f", "lavfi", "-i", "sine=f=440:d=30", "-c:v", "libx264", "-c:a", "aac", "-f", "mpegts", tsPath)
...
func TestHLSEndpoint(t *testing.T) {
	...
	t.Run("WithFFmpeg_Repair", func(t *testing.T) {
		...
		if !strings.Contains(sBody, ".ts") {
			// It might be empty if stream probing failed or taking time?
			// HLSManager creates playlist immediately?
			// It might yield empty playlist if no segments yet.
			// Users loop until segments appear?
			// Integration test might need to wait or just accept generic structure.
			// "Teste die Segmente ... sollte 200 sein" implies segments exist.
			// If we mocked TS, HLSManager should read it and produce segments.
