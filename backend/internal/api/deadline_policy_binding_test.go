...
func TestCanonicalBaselineParity(t *testing.T) {
	...
	require.Len(t, expected, 157)
	require.Len(t, actual, 157)
	...
	require.Equal(t, 27, counts["outer"])
...
func TestPolicyBindingSnapshotTracksBuildSpecificUIVariant(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, 157, snapshot.Len())
...
func TestPhase2RuntimeReadinessAll103Routes(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, registrations, 158)
	...
	require.Equal(t, 158, runtimeReady)
}
