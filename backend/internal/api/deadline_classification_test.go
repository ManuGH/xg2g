...
func TestDeadlineClassification_DevProxyReportsPendingHijackVerification(t *testing.T) {
	...
	assert.Equal(t, 140, totalCount)
	assert.Equal(t, 140, structuralCount)
	assert.Equal(t, 140, declaredCount)
	assert.Equal(t, 138, runtimeReadyBoundedOrStreamingCount, "all 138 bounded and streaming routes must be runtime-ready with verified evidence")
	...
	for _, variant := range variants {
		t.Run(string(variant), func(t *testing.T) {
			...
			if variant == ConfigVariantDevProxy {
				assert.Equal(t, 140, len(regs), "total registrable instances must equal 140 under DevProxy")
				assert.Equal(t, 140, apiBounded+mediaBounded+streaming)
				assert.Equal(t, 12, mediaBounded, "RouteDeadlineMediaBounded count is 12 under DevProxy")
				assert.Equal(t, 126, apiBounded, "RouteDeadlineAPIBounded count is 126 under DevProxy")
				assert.Equal(t, 2, mayUpgradeCount, "DevProxy has 2 MayUpgradePerRequest routes (GET /ui and GET /ui/*)")
			} else {
				assert.Equal(t, 140, len(regs), "total registrable instances must equal 140 under ProdStatic/DevDir")
				assert.Equal(t, 140, apiBounded+mediaBounded+streaming)
				assert.Equal(t, 13, mediaBounded, "RouteDeadlineMediaBounded count is 13 under ProdStatic/DevDir")
				assert.Equal(t, 125, apiBounded, "RouteDeadlineAPIBounded count is 125 under ProdStatic/DevDir")
...
func TestDeadlineClassification_RawInventoryDiagnostics(t *testing.T) {
	...
	assert.Equal(t, 114, rawV3Count, "raw v3 walk contains 114 routes")
	...
	assert.Equal(t, 140, (rawOuterCount-filteredDelegateCount)+rawV3Count)
}
