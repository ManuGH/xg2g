...
func TestDeadlineClassification_DevProxyReportsPendingHijackVerification(t *testing.T) {
	...
	assert.Equal(t, 159, totalCount)
	assert.Equal(t, 159, structuralCount)
	assert.Equal(t, 159, declaredCount)
	assert.Equal(t, 157, runtimeReadyBoundedOrStreamingCount, "all 157 bounded and streaming routes must be runtime-ready with verified evidence")
	...
	for _, variant := range variants {
		t.Run(string(variant), func(t *testing.T) {
			...
			if variant == ConfigVariantDevProxy {
				assert.Equal(t, 159, len(regs), "total registrable instances must equal 159 under DevProxy")
				assert.Equal(t, 159, apiBounded+mediaBounded+streaming)
				...
				assert.Equal(t, 159, len(regs), "total registrable instances must equal 159 under ProdStatic/DevDir")
				assert.Equal(t, 159, apiBounded+mediaBounded+streaming)
...
func TestDeadlineClassification_RawInventoryDiagnostics(t *testing.T) {
	...
	assert.Equal(t, 37, rawOuterCount, "raw outer walk contains 28 routes + 9 method expansions of /api/v3/* delegate mount")
	...
	assert.Equal(t, 28, rawOuterCount-filteredDelegateCount)
	assert.Equal(t, 159, (rawOuterCount-filteredDelegateCount)+rawV3Count)
}
