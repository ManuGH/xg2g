...
func TestDeadlineClassification_DevProxyReportsPendingHijackVerification(t *testing.T) {
	...
	assert.Equal(t, 162, totalCount)
	assert.Equal(t, 162, structuralCount)
	assert.Equal(t, 162, declaredCount)
	assert.Equal(t, 160, runtimeReadyBoundedOrStreamingCount, "all 160 bounded and streaming routes must be runtime-ready with verified evidence")
	...
	for _, variant := range variants {
		t.Run(string(variant), func(t *testing.T) {
			...
			if variant == ConfigVariantDevProxy {
				assert.Equal(t, 162, len(regs), "total registrable instances must equal 162 under DevProxy")
				assert.Equal(t, 162, apiBounded+mediaBounded+streaming)
				assert.Equal(t, 12, mediaBounded, "RouteDeadlineMediaBounded count is 12 under DevProxy")
				assert.Equal(t, 145, apiBounded, "RouteDeadlineAPIBounded count is 145 under DevProxy")
				...
				assert.Equal(t, 162, len(regs), "total registrable instances must equal 162 under ProdStatic/DevDir")
				assert.Equal(t, 162, apiBounded+mediaBounded+streaming)
				assert.Equal(t, 13, mediaBounded, "RouteDeadlineMediaBounded count is 13 under ProdStatic/DevDir")
				assert.Equal(t, 144, apiBounded, "RouteDeadlineAPIBounded count is 144 under ProdStatic/DevDir")
...
func TestDeadlineClassification_RawInventoryDiagnostics(t *testing.T) {
	...
	t.Logf("Raw Outer Walk Entries: %d (30 classifiable routes + %d filtered delegate mount methods)", rawOuterCount, filteredDelegateCount)
	...
	assert.Equal(t, 132, rawV3Count, "raw v3 walk contains 132 routes")
	...
	assert.Equal(t, 162, (rawOuterCount-filteredDelegateCount)+rawV3Count)
}
