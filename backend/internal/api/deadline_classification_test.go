...
func TestDeadlineClassification_DevProxyReportsPendingHijackVerification(t *testing.T) {
	...
	assert.Equal(t, 167, totalCount)
	assert.Equal(t, 167, structuralCount)
	assert.Equal(t, 167, declaredCount)
	assert.Equal(t, 165, runtimeReadyBoundedOrStreamingCount, "all 165 bounded and streaming routes must be runtime-ready with verified evidence")
	...
	for _, variant := range variants {
		t.Run(string(variant), func(t *testing.T) {
			...
			if variant == ConfigVariantDevProxy {
				assert.Equal(t, 167, len(regs), "total registrable instances must equal 167 under DevProxy")
				assert.Equal(t, 167, apiBounded+mediaBounded+streaming)
				assert.Equal(t, 12, mediaBounded, "RouteDeadlineMediaBounded count is 12 under DevProxy")
				assert.Equal(t, 150, apiBounded, "RouteDeadlineAPIBounded count is 150 under DevProxy")
				...
				assert.Equal(t, 167, len(regs), "total registrable instances must equal 167 under ProdStatic/DevDir")
				assert.Equal(t, 167, apiBounded+mediaBounded+streaming)
				assert.Equal(t, 13, mediaBounded, "RouteDeadlineMediaBounded count is 13 under ProdStatic/DevDir")
				assert.Equal(t, 149, apiBounded, "RouteDeadlineAPIBounded count is 149 under ProdStatic/DevDir")
...
func TestDeadlineClassification_RawInventoryDiagnostics(t *testing.T) {
	...
	assert.Equal(t, 137, rawV3Count, "raw v3 walk contains 137 routes")
	...
	assert.Equal(t, 167, (rawOuterCount-filteredDelegateCount)+rawV3Count)
}
