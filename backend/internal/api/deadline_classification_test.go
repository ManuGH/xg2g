...
func TestDeadlineClassification_DevProxyReportsPendingHijackVerification(t *testing.T) {
	...
	assert.Equal(t, 118, totalCount)
	assert.Equal(t, 118, structuralCount)
	assert.Equal(t, 118, declaredCount)
	assert.Equal(t, 116, runtimeReadyBoundedOrStreamingCount, "all 116 bounded and streaming routes must be runtime-ready with verified evidence")
...
func TestDeadlineClassification_InventoryCountsAndClassificationList(t *testing.T) {
	...
	for _, variant := range variants {
		t.Run(string(variant), func(t *testing.T) {
			...
			assert.Equal(t, 118, len(regs), "total registrable instances must equal 118")
			...
			assert.Equal(t, 118, apiBounded+mediaBounded+streaming)
			...
			if variant == ConfigVariantDevProxy {
				...
				assert.Equal(t, 104, apiBounded, "RouteDeadlineAPIBounded count is 104 under DevProxy")
				...
				assert.Equal(t, 103, apiBounded, "RouteDeadlineAPIBounded count is 103 under ProdStatic/DevDir")
...
func TestDeadlineClassification_RawInventoryDiagnostics(t *testing.T) {
	...
	assert.Equal(t, 92, rawV3Count, "raw v3 walk contains 92 routes")
	...
	assert.Equal(t, 118, (rawOuterCount-filteredDelegateCount)+rawV3Count)
}
