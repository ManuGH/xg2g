...
func TestDeadlineClassification_DevProxyReportsPendingHijackVerification(t *testing.T) {
	...
	assert.Equal(t, 158, totalCount)
	assert.Equal(t, 158, structuralCount)
	assert.Equal(t, 158, declaredCount)
	assert.Equal(t, 156, runtimeReadyBoundedOrStreamingCount, "all 156 bounded and streaming routes must be runtime-ready with verified evidence")
	...
	for _, variant := range variants {
		t.Run(string(variant), func(t *testing.T) {
			...
			if variant == ConfigVariantDevProxy {
				assert.Equal(t, 158, len(regs), "total registrable instances must equal 158 under DevProxy")
				assert.Equal(t, 158, apiBounded+mediaBounded+streaming)
				...
				assert.Equal(t, 4, streaming, "RouteDeadlineStreaming count is 4 under DevProxy")
				assert.Equal(t, 2, mayUpgradeCount, "DevProxy has 2 MayUpgradePerRequest routes (GET /ui and GET /ui/*)")
			} else {
				assert.Equal(t, 158, len(regs), "total registrable instances must equal 158 under ProdStatic/DevDir")
				assert.Equal(t, 158, apiBounded+mediaBounded+streaming)
				...
				assert.Equal(t, 4, streaming, "RouteDeadlineStreaming count is 4 under ProdStatic/DevDir")
...
func TestDeadlineClassification_RawInventoryDiagnostics(t *testing.T) {
	...
	t.Logf("Raw Outer Walk Entries: %d (27 classifiable routes + %d filtered delegate mount methods)", rawOuterCount, filteredDelegateCount)
	...
	assert.Equal(t, 36, rawOuterCount, "raw outer walk contains 27 routes + 9 method expansions of /api/v3/* delegate mount")
	...
	assert.Equal(t, 27, rawOuterCount-filteredDelegateCount)
	assert.Equal(t, 158, (rawOuterCount-filteredDelegateCount)+rawV3Count)
}
