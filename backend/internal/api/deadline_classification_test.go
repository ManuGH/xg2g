...
func TestDeadlineClassification_DevProxyReportsPendingHijackVerification(t *testing.T) {
	...
	assert.Equal(t, 130, totalCount)
	assert.Equal(t, 130, structuralCount)
	assert.Equal(t, 130, declaredCount)
	assert.Equal(t, 128, runtimeReadyBoundedOrStreamingCount, "all 128 bounded and streaming routes must be runtime-ready with verified evidence")
	...
	for _, variant := range variants {
		t.Run(string(variant), func(t *testing.T) {
			...
			if variant == ConfigVariantDevProxy {
				assert.Equal(t, 130, len(regs), "total registrable instances must equal 130 under DevProxy")
				assert.Equal(t, 130, apiBounded+mediaBounded+streaming)
				assert.Equal(t, 12, mediaBounded, "RouteDeadlineMediaBounded count is 12 under DevProxy")
				assert.Equal(t, 116, apiBounded, "RouteDeadlineAPIBounded count is 116 under DevProxy")
				assert.Equal(t, 2, mayUpgradeCount, "DevProxy has 2 MayUpgradePerRequest routes (GET /ui and GET /ui/*)")
			} else {
				assert.Equal(t, 130, len(regs), "total registrable instances must equal 130 under ProdStatic/DevDir")
				assert.Equal(t, 130, apiBounded+mediaBounded+streaming)
				assert.Equal(t, 13, mediaBounded, "RouteDeadlineMediaBounded count is 13 under ProdStatic/DevDir")
				assert.Equal(t, 115, apiBounded, "RouteDeadlineAPIBounded count is 115 under ProdStatic/DevDir")
...
func TestDeadlineClassification_RawInventoryDiagnostics(t *testing.T) {
	...
	assert.Equal(t, 104, rawV3Count, "raw v3 walk contains 104 routes")
	...
	assert.Equal(t, 130, (rawOuterCount-filteredDelegateCount)+rawV3Count)
}
