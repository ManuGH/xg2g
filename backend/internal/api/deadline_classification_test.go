...
func TestDeadlineClassification_DevProxyReportsPendingHijackVerification(t *testing.T) {
	...
	assert.Equal(t, 116, totalCount)
	assert.Equal(t, 116, structuralCount)
	assert.Equal(t, 116, declaredCount)
	assert.Equal(t, 114, runtimeReadyBoundedOrStreamingCount, "all 114 bounded and streaming routes must be runtime-ready with verified evidence")
...
func TestDeadlineClassification_InventoryCountsAndClassificationList(t *testing.T) {
	...
	for _, variant := range variants {
		t.Run(string(variant), func(t *testing.T) {
			...
			assert.Equal(t, 116, len(regs), "total registrable instances must equal 116")
			...
			assert.Equal(t, 116, apiBounded+mediaBounded+streaming)
			...
			if variant == ConfigVariantDevProxy {
				...
				assert.Equal(t, 102, apiBounded, "RouteDeadlineAPIBounded count is 102 under DevProxy")
				...
				assert.Equal(t, 101, apiBounded, "RouteDeadlineAPIBounded count is 101 under ProdStatic/DevDir")
...
func TestDeadlineClassification_RawInventoryDiagnostics(t *testing.T) {
	...
	assert.Equal(t, 90, rawV3Count, "raw v3 walk contains 90 routes")
	...
	assert.Equal(t, 116, (rawOuterCount-filteredDelegateCount)+rawV3Count)
}
