...
func buildDSN(dbPath string, cfg Config) string {
	...
	// modernc.org/sqlite supports both _pragma and _txlock in the DSN.
	// Format: file:path?_txlock=immediate&_pragma=foo(bar)&_pragma=baz(qux)
	return fmt.Sprintf("file:%s?_txlock=%s&_pragma=journal_mode(WAL)&_pragma=busy_timeout(%d)&_pragma=synchronous(NORMAL)&_pragma=foreign_keys(ON)",
		dbPath, txLock, cfg.BusyTimeout.Milliseconds())
}
