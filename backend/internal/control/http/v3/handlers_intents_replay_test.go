...
func TestHandleV3Intents_StoppingReplayStartsFreshSession(t *testing.T) {
	s, st := newV3TestServer(t, t.TempDir())
	...
	send := func() v3api.IntentResponse {
		body := intentBodyWithValidJWT(t, svcRef, "", "live", "corr-replay-stopping-001")
		req := httptest.NewRequest(http.MethodPost, V3BaseURL+"/intents", bytes.NewReader(body))
		req.Header.Set("Authorization", "Bearer test-token")
		req.Header.Set("Content-Type", "application/json")
	...
	// Session transitions to STOPPING (e.g., when client issued stream.stop)
	_, err = st.UpdateSession(context.Background(), first.SessionID, func(sess *model.SessionRecord) error {
		sess.State = model.SessionStopping
		sess.Reason = model.RClientStop
		return nil
	})
	require.NoError(t, err)

	// Second start intent within the idempotency window must NOT replay the stopping session
	second := send()
	require.Equal(t, "accepted", second.Status)
	require.NotEqual(t, first.SessionID, second.SessionID, "stopping session must not be replayed; fresh session must be created")

	select {
	case <-sub.C():
	case <-time.After(200 * time.Millisecond):
		t.Fatal("expected second session.start event after stopping replay cleanup")
	}
