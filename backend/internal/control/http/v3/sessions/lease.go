...
func (s *Service) RenewLeaseFromConsumption(ctx context.Context, sessionID string) {
	if s == nil || s.deps == nil {
		return
	}
	...
	if interval <= 0 {
		if d := s.deps.Config().Sessions.HeartbeatInterval; d > 0 {
	...
	ttl := s.deps.Config().Sessions.LeaseTTL
	if ttl <= 0 {
		return
	}
