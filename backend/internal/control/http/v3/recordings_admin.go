...
func updateRecordingMetaTitle(localPath, title string) error {
	...
	if err := os.WriteFile(tmpPath, []byte(updated), mode); err != nil { // #nosec G304,G703 -- tmpPath derives from resolver-confined localPath (same path justified for G304/G703 above)
