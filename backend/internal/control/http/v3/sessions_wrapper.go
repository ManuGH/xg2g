...
// GetSessionEvents implements GET /sessions/{sessionID}/events.
func (s *Server) GetSessionEvents(w http.ResponseWriter, r *http.Request, sessionID openapi_types.UUID) {
	_ = sessionID
	s.ScopeMiddleware(ScopeV3Read)(http.HandlerFunc(s.handleV3SessionEvents)).ServeHTTP(w, r)
}
