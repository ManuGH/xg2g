					...
					if o.TopologyService != nil {
						if ok := o.TopologyService.HeartbeatStream(event.SessionID, o.LeaseTTL); !ok {
							if o.TopologyService.Mode() == receivertopology.EvaluationModeEnforce &&
								...
								logger.Warn().Str("sid", event.SessionID).Msg("topology stream lease lost in enforce mode, aborting session")
							...
							logger.Warn().
								...
								Str("mode", string(o.TopologyService.Mode())).
								Str("confidence", string(o.TopologyService.Topology().Confidence)).
								Msg("topology stream lease missing in audit-only mode (fail-open, continuing session)")
						}
					}
