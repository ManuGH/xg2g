...
// transformArgsForAvsyncPipe rewrites a normal live-copy argv for stdin (pipe:0)
// input: the HTTP input URL is replaced by pipe:0, HTTP-only input options are
// dropped (FFmpeg rejects/ignores them for a pipe), and a leading-audio atrim is
// inserted before the audio encoder so copied video and transcoded audio share an
// origin. Pure function: unit-tested independently of the live peek.
func transformArgsForAvsyncPipe(args []string, orphan float64) []string {
	return transformArgsForAvsyncPipeMode(args, orphan, true)
}
