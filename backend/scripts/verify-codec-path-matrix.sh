...
run go test ./internal/control/http/v3/intents -run 'Test(ApplyClientCompatibilityPolicy_IOSSafariNativeHEVC(DemotesToEncodeOnly|KeepsFullVAAPIWhenConfigured|FallsBackToCPUWhenConfigured)|Service_ProcessIntent_Start(UsesEncodeOnlyForIOSSafariNativeHEVC|AllowsFullVAAPIForIOSSafariNativeHEVCWhenConfigured|UsesProgressiveScanTruthForAV1HW))$' -count=1
