...
if grep -r "as any" apps/webui/src --include="*.tsx" --include="*.ts" --exclude-dir="client-ts" --exclude-dir="node_modules" --exclude="*.test.tsx" --exclude="*.spec.tsx" --exclude="*.test.ts"; then
...
if grep -rE "\.ref\b" apps/webui/src --include="*.tsx" --include="*.ts" --exclude-dir="client-ts" --exclude-dir="node_modules" | grep -v "useRef"; then
...
if grep -r "bouquets: string\[\]" apps/webui/src/features/epg; then
