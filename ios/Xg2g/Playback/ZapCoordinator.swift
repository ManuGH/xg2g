...
final class ZapCoordinator {
    ...
    private(set) var presentedServiceRef: String?
    ...
    private(set) var displayedServiceRef: String?
    ...
    private(set) var requestedServiceRef: String?
    ...
    private(set) var playing: NativeTSVideoPipeline?

    /// The visible surface. The screen hosts its layer; it never decides what goes on it.
    @ObservationIgnored let surface: SystemVideoPresenter

    /// Who the surface belongs to.
    @ObservationIgnored let context: PresentationContext
    ...
    @ObservationIgnored private var visibleSessionObserver: AnyCancellable?
    ...
    @ObservationIgnored private var activeZapID: String?

    @ObservationIgnored private var zapCounter = 0
    ...
    @ObservationIgnored private let preparationsProvider: (@MainActor () -> ZapPreparationClient?)?
    ...
    @ObservationIgnored private let streamURL: @MainActor (String) -> URL?
    @ObservationIgnored private let makeSession: @MainActor () -> NativeTSVideoPipeline
    ...
    func zap(to serviceRef: String) async {
        ...
        @MainActor final class RetirementGate {
            var hasRetired = false
        }
        let retirementGate = RetirementGate()
        ...
        session.onFirstPictureVisible = { [weak self, weak session, weak retiring] in
            Task { @MainActor [weak self, weak session, weak retiring] in
                ...
                if !retirementGate.hasRetired {
                    retirementGate.hasRetired = true
    ...
    private func follow(_ session: NativeTSVideoPipeline) {
        visibleSessionObserver = session.objectWillChange.sink { [weak self] _ in
            self?.withMutation(keyPath: \.playing) {}
        }
    }
