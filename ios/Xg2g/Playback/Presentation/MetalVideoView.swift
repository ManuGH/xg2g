...
public final class MetalVideoView: UIView {
    ...
    public override func layoutSubviews() {
        ...
        let scale = window?.screen.scale ?? (traitCollection.displayScale > 0 ? traitCollection.displayScale : 2.0)
    ...
    private func setupMetal() {
        ...
        metalLayer.contentsScale = traitCollection.displayScale > 0 ? traitCollection.displayScale : 2.0
    ...
    private func encodeDeinterlaceToNV12(
        ...
        // Luma cleared to video black (16/255 ≈ 0.06275); Chroma cleared to neutral grey (128/255 ≈ 0.50196)
        // to prevent blue/magenta edge flashes from uninitialized TBDR tile memory.
        guard encodePlane(target: lumaTarget, pipeline: lumaPipeline, texture: source.luma, clearColor: MTLClearColorMake(0.06275, 0.0, 0.0, 1.0)),
              encodePlane(target: chromaTarget, pipeline: chromaPipeline, texture: source.chroma, clearColor: MTLClearColorMake(0.50196, 0.50196, 0.0, 1.0)) else {
            return false
        }
    ...
    private func presentThroughSystemLayer(_ field: PendingField, presenter: SystemVideoPresenter) {
        ...
        // Destination buffer is now a fully reconstructed progressive NV12 frame.
        // Remove field detail and mark fieldCount = 1 so AVSampleBufferDisplayLayer
        // does not attempt duplicate deinterlacing or field display.
        CVBufferRemoveAttachment(destination, kCVImageBufferFieldDetailKey)
        CVBufferRemoveAttachment(destination, kCVImageBufferChromaLocationBottomFieldKey)
