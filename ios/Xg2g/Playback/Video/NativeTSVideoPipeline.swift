...
public final class NativeTSVideoPipeline: NSObject, ObservableObject, @unchecked Sendable,
    ...
    private let silencedLock = NSLock()
    private var _isSilenced = false
    private var isSilenced: Bool {
        get {
            silencedLock.lock()
            defer { silencedLock.unlock() }
            return _isSilenced
        }
        set {
            silencedLock.lock()
            _isSilenced = newValue
            silencedLock.unlock()
        }
    }
    ...
    private static let audioPreRollSeconds: Double = 1.0
    ...
    private static let ingestQueueSpecificKey = DispatchSpecificKey<UUID>()
    ...
    private var lastAudioStarvationRecoveryTime: CFTimeInterval = 0
    ...
    public init(audioOutput: PlaybackAudioOutput) {
        ...
        super.init()
        ingestQueue.setSpecific(key: Self.ingestQueueSpecificKey, value: pipelineId)
        self.finishInit()
    }

    /// Safely executes work on `ingestQueue`, executing inline if already on `ingestQueue`
    /// to avoid deadlocks (e.g. during `deinit` when tearing down).
    @inline(__always)
    private func performOnIngestQueue<T>(_ work: () throws -> T) rethrows -> T {
        if DispatchQueue.getSpecific(key: Self.ingestQueueSpecificKey) == pipelineId {
            return try work()
        } else {
            return try ingestQueue.sync(execute: work)
        }
    }
    ...
    /// Whether any discovered audio track on this stream can be decoded on device (e.g. AAC, AC-3, E-AC-3).
    public var hasDecodableAudio: Bool {
        sessionState.mutate { $0.hasDecodableAudio }
    }

    /// Whether audio tracks have already been discovered from the stream's PMT.
    public var hasAudioTracksKnown: Bool {
        sessionState.mutate { $0.audioTracksKnown }
    }
    ...
    func ingest(_ ingest: LiveStreamIngest, didReceive data: Data) {
        ...
        if now.timeIntervalSince(lastBitrateCheck) >= 1.0 {
            ...
            // Audio Starvation Watchdog:
            // If the audio clock is running and the cushion has completely collapsed (negative lead / chronic underruns),
            // re-anchor to let the buffer replenish rather than remaining in perpetual stuttering.
            let nowMono = CACurrentMediaTime()
            if self.isAudioClockStarted && (audio.minLeadMs < -40.0 || audio.currentLeadMs < 0.0) && (nowMono - self.lastAudioStarvationRecoveryTime > 10.0) {
                self.lastAudioStarvationRecoveryTime = nowMono
                self.ingestQueue.async { [weak self] in
                    self?.handleAudioStarvation(leadMs: audio.currentLeadMs, minLeadMs: audio.minLeadMs)
                }
            }
    ...
    public func tsParser(_ parser: TSPacketParser, didEncounterContinuityErrorOnPID pid: UInt16, expected: UInt8, actual: UInt8) {
        ...
        if let vPid = parser.videoPID, pid == vPid {
            pesAssembler.handleContinuityError()
            accessUnitAssembler.handleContinuityError()
            alternateAssembler?.handleContinuityError()
            decodeGateState = .closed(reason: .decoderRecovery)
            gatedAccessUnitCount = 0
        } else if parser.audioPIDs.contains(pid) || pid == selectedAudioPID {
            audioPesAssembler.handleContinuityError(onPID: pid)
            if pid == selectedAudioPID {
                if selectedAudioCodec == .aac {
                    aacFrameParser.handleContinuityError()
                } else {
                    ac3FrameParser.handleContinuityError()
                }
            }
        } else if let sub = selectedSubtitleTrack,
           pid == sub.pid,
           case .dvb = sub.format {
    ...
    public func audioSampleBufferAssembler(_ assembler: AudioSampleBufferAssembler, didEmitSampleBuffer sampleBuffer: CMSampleBuffer, codec: AudioStreamCodec, duration: CMTime) {
        guard !isAudioInterrupted, !isSilenced else { return }
        ...
        if !isAudioClockStarted {
            ...
            // Recorded once the anchor is final AND has sufficient audio cushion (>= minRequiredAudioLead)
            // to guarantee glitch-free continuous playback without starving CoreAudio.
    ...
    private func handleAudioStarvation(leadMs: Double, minLeadMs: Double) {
        performOnIngestQueue {
            guard self.isAudioClockStarted else { return }
            let msg = "[1080i50-AUDIO] ⚠️ Severe audio starvation detected (lead: \(String(format: "%.0f", leadMs))ms, min: \(String(format: "%.0f", minLeadMs))ms) — re-anchoring master clock with fresh cushion"
            print(msg)
            logger.warning("\(msg, privacy: .public)")
            TelemetryServer.shared.log(msg)

            self.beginRecovery("audio starvation (lead \(String(format: "%.0f", leadMs))ms)")
            self.audioRenderer.flush()
            self.audioRenderer.setRate(0.0, time: .invalid)
