...
public final class NativeTSAudioRenderer: @unchecked Sendable {
    ...
    public func pruneBuffersBefore(time anchor: CMTime) -> PruneResult {
        ...
        let lastBufferPTS = pendingBuffers.last.map { buffer -> CMTime in
            let pts = CMSampleBufferGetPresentationTimeStamp(buffer)
            let duration = CMSampleBufferGetDuration(buffer)
            return (pts.isValid && duration.isValid) ? CMTimeAdd(pts, duration) : pts
        }
        let latestAudioEnd = lastBufferPTS ?? enqueuedEndPTS
        let remainingLeadMs: Double
        if let latest = latestAudioEnd, latest.isValid, anchor.isValid {
            remainingLeadMs = max(0, (latest.seconds - anchor.seconds) * 1000.0)
        } else {
            remainingLeadMs = 0
        }
    ...
    /// Stops playback, flushes queued samples, halts clock and mutes without tearing down the renderer.
    public func stop() {
        setAudible(false)
        stopClock()
        flush()
