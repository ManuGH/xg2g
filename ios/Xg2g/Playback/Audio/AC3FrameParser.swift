...
public final class AC3FrameParser: @unchecked Sendable {
    ...
    /// Handles a transport stream continuity error on the audio PID.
    /// Discards broken partial frames and clears pending PTS for clean re-anchoring.
    ...
    public func feed(data: Data, pts: CMTime? = nil, pts90k: UInt64? = nil) {
        if let pts = pts, pts.isValid {
            if let current = self.pendingPTS {
                let delta = pts.seconds - current.seconds
                // If the delta is within normal continuity tolerance (< 50ms),
                // do NOT overwrite pendingPTS. Keep the continuous exact frame cadence!
                // Only resync if there is a genuine timeline jump or discontinuity (> 50ms).
                if abs(delta) > 0.050 {
                    buffer.removeAll(keepingCapacity: true)
                    self.pendingPTS = pts
