...
struct AACFrameParserTests {
    ...
    // MARK: - 4. Continuous PTS Cadence Preservation Across PES Chunks

    @Test func preservesContinuousCadenceAcrossPESArrivalsWithSmallCadenceJitter() throws {
        let parser = AACADTSFrameParser()
        let sink = MockAACFrameSink()
        parser.delegate = sink

        let frame1 = makeADTSFrame(profile: 1, sampleRateIdx: 3, channelConfig: 2, frameLength: 200)
        let frame2 = makeADTSFrame(profile: 1, sampleRateIdx: 3, channelConfig: 2, frameLength: 200)

        // Feed first frame at PTS 10.0s
        let startPTS = CMTime(value: 900000, timescale: 90000)
        parser.feed(data: frame1, pts: startPTS, pts90k: 900000)

        #expect(sink.emittedFrames.count == 1)
        #expect(sink.emittedFrames[0].pts?.seconds == 10.0)

        // Feed second frame in a separate PES with slightly misaligned packet PTS (e.g. +30ms instead of exact +21.33ms)
        // Cadence protection (< 50ms) must ignore the jittered PES PTS and preserve continuous frame spacing!
        let jitteredPESPTS = CMTime(value: 902700, timescale: 90000) // 10.030s (+30ms)
        parser.feed(data: frame2, pts: jitteredPESPTS, pts90k: 902700)

        #expect(sink.emittedFrames.count == 2)
        let expectedPTS = 10.0 + (1024.0 / 48000.0) // 10.021333...
        #expect(abs((sink.emittedFrames[1].pts?.seconds ?? 0) - expectedPTS) < 0.0001)
    }

    // MARK: - 5. Continuity Error Handling

    @Test func handleContinuityErrorDiscardsCorruptedBufferAndPendingPTS() throws {
        let parser = AACADTSFrameParser()
        let sink = MockAACFrameSink()
        parser.delegate = sink

        let frame = makeADTSFrame(profile: 1, sampleRateIdx: 3, channelConfig: 2, frameLength: 200)
        // Feed partial frame (first 100 bytes)
        parser.feed(data: frame.prefix(100), pts: CMTime(value: 90000, timescale: 90000))
        #expect(sink.emittedFrames.isEmpty)

        // Continuity error occurs
        parser.handleContinuityError()

        // Feed second half of the previous frame — should NOT assemble into a Frankenstein frame
        parser.feed(data: frame.suffix(100))
        #expect(sink.emittedFrames.isEmpty)

        // Feed clean full frame with new PTS
        let cleanPTS = CMTime(value: 180000, timescale: 90000)
        parser.feed(data: frame, pts: cleanPTS, pts90k: 180000)
        #expect(sink.emittedFrames.count == 1)
        #expect(sink.emittedFrames[0].pts?.seconds == 2.0)
    }
}
