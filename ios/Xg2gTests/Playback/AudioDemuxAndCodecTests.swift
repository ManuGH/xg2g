...
struct AudioDemuxAndCodecTests {
    ...
    @Test func audioPESAssemblerDropsCorruptPESOnContinuityError() throws {
        let assembler = AudioPESAssembler()
        let sink = MockAudioPESSink()
        assembler.delegate = sink

        let testPID: UInt16 = 401
        // Feed start of an audio PES packet
        let payload1 = Data([0x0B, 0x77, 0x01, 0x02])
        let pes1 = makeAudioPESPacket(streamID: 0xBD, pts90k: 90000, payload: payload1)
        assembler.feed(payload: pes1, pid: testPID, unitStart: true)

        // Continuity error occurs mid-stream
        assembler.handleContinuityError(onPID: testPID)

        // Continuation packet across the drop gap must be discarded
        let continuation = Data([0xAA, 0xBB, 0xCC, 0xDD])
        assembler.feed(payload: continuation, pid: testPID, unitStart: false)

        // Flush must NOT emit the broken packet
        assembler.flush(pid: testPID)
        #expect(sink.emittedPackets.isEmpty)

        // Next clean audio PES packet on new PUSI
        let payload2 = Data([0x0B, 0x77, 0x03, 0x04])
        let pes2 = makeAudioPESPacket(streamID: 0xBD, pts90k: 93600, payload: payload2)
        assembler.feed(payload: pes2, pid: testPID, unitStart: true)
        assembler.flush(pid: testPID)

        #expect(sink.emittedPackets.count == 1)
        #expect(sink.emittedPackets.first?.payload == payload2)
    }
}
