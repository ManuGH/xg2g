...
@MainActor
@Suite struct TimeshiftTransitionTests {
    ...
    func testPlaybackEngineModeEquality() {
        let liveMode = LivePlayerScreen.PlaybackEngineMode.nativeDirectLive
        let timeshiftMode = LivePlayerScreen.PlaybackEngineMode.timeshiftHLS

        #expect(liveMode == .nativeDirectLive)
        #expect(timeshiftMode == .timeshiftHLS)
        #expect(liveMode != timeshiftMode)
        #expect(TestTSPlayerScreen.self == LivePlayerScreen.self)
    }
}
