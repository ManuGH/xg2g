...
struct TSPipelineUnitTests {
    ...
    @Test func pesAssemblerDropsCorruptPESOnContinuityError() throws {
        let assembler = PESPacketAssembler()
        let sink = MockPESSink()
        assembler.delegate = sink

        // Feed start of a video PES packet
        var pesPart1 = Data([0x00, 0x00, 0x01, 0xE0, 0x00, 0x00, 0x80, 0x00, 0x00])
        pesPart1.append(Data([0x00, 0x00, 0x00, 0x01, 0x41, 0x9A]))
        assembler.feed(payload: pesPart1, unitStart: true)

        // A continuity error occurs mid-PES (packet loss over network / tuner)
        assembler.handleContinuityError()

        // Continuation packet arrives across the hole - must be dropped
        let corruptedContinuation = Data([0x00, 0x00, 0x00, 0x01, 0x41, 0x9B])
        assembler.feed(payload: corruptedContinuation, unitStart: false)

        // Next clean PES begins on a new PUSI
        var nextCleanPES = Data([0x00, 0x00, 0x01, 0xE0, 0x00, 0x00, 0x80, 0x00, 0x00])
        let cleanNAL = Data([0x00, 0x00, 0x00, 0x01, 0x05, 0x88])
        nextCleanPES.append(cleanNAL)
        assembler.feed(payload: nextCleanPES, unitStart: true)

        // Close the clean PES
        assembler.feed(payload: Data([0x00, 0x00, 0x01, 0xE0]), unitStart: true)

        // Corrupted PES must not have been emitted; only clean PES should be emitted
        #expect(sink.emittedPayloads.count == 1)
        #expect(sink.emittedPayloads.first?.data == cleanNAL)
    }
