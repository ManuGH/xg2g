...
**Result**
- Introduced wrapper module at `apps/webui/src/client-ts/wrapper.ts`.
- Product code now consumes generated client only via `apps/webui/src/client-ts` exports or wrapper helpers.
- Added typed RFC7807/API error mapping tests at `apps/webui/src/client-ts/wrapper.test.ts`.
- Added guard script `apps/webui/scripts/verify-client-wrapper-boundary.sh` and wired it into the WebUI lint workflow.
...
**Result**
- Added/extended `apps/webui/scripts/verify-client-wrapper-boundary.sh` with full-scan + diff-scoped modes.
- Wired guard into:
