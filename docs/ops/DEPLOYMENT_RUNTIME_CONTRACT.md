...
services:
  xg2g:
    image: ghcr.io/manugh/xg2g:v3.9.3
    container_name: xg2g
...
```bash
docker run --rm xg2g:v3.9.3 which ffmpeg
```
...
```bash
docker run --rm xg2g:v3.9.3 ffmpeg -version | head -1
```
...
```bash
docker run --rm xg2g:v3.9.3 sh -c 'echo $XG2G_FFMPEG_BIN'
```
...
```bash
docker run --rm -e FFMPEG_HOME=/nonexistent xg2g:v3.9.3 ffmpeg -version
```
...
```bash
docker run --rm --device /dev/dri:/dev/dri xg2g:v3.9.3 \
...
docker inspect --format='{{.Config.User}}' xg2g:v3.9.3
```

Test (no device):

```bash
docker run --rm xg2g:v3.9.3 \
...
```bash
docker run --rm --gpus all xg2g:v3.9.3 \
...
- name: Verify Deployment Contract
  run: |
    # FFmpeg wrapper
    docker run --rm xg2g:v3.9.3 sh -c '
