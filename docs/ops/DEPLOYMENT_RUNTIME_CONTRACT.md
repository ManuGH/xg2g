...
services:
  xg2g:
    image: ghcr.io/manugh/xg2g:v3.4.1
    container_name: xg2g
...
```bash
docker run --rm xg2g:v3.4.1 which ffmpeg
```
...
```bash
docker run --rm xg2g:v3.4.1 ffmpeg -version | head -1
```
...
```bash
docker run --rm xg2g:v3.4.1 sh -c 'echo $XG2G_FFMPEG_BIN'
```
...
```bash
docker run --rm -e FFMPEG_HOME=/nonexistent xg2g:v3.4.1 ffmpeg -version
```
...
```bash
docker run --rm --device /dev/dri/renderD128 xg2g:v3.4.1 ls -l /dev/dri/
```
...
```bash
docker run --rm --device /dev/dri/renderD128 xg2g:v3.4.1 \
...
docker inspect --format='{{.Config.User}}' xg2g:v3.4.1
```

Test (no device):

```bash
docker run --rm xg2g:v3.4.1 \
  ffmpeg -hwaccel vaapi -hwaccel_device /dev/dri/renderD128 -f lavfi -i testsrc -t 1 -f null -
```
...
- name: Verify Deployment Contract
  run: |
    # FFmpeg wrapper
    docker run --rm xg2g:v3.4.1 sh -c '
