...
services:
  xg2g:
    image: ghcr.io/manugh/xg2g:v3.9.6
    container_name: xg2g
...
```bash
docker run --rm xg2g:v3.9.6 which ffmpeg
```
...
```bash
docker run --rm xg2g:v3.9.6 ffmpeg -version | head -1
```
...
```bash
docker run --rm xg2g:v3.9.6 sh -c 'echo $XG2G_FFMPEG_BIN'
```
...
```bash
docker run --rm -e FFMPEG_HOME=/nonexistent xg2g:v3.9.6 ffmpeg -version
```
...
```bash
docker run --rm --device /dev/dri:/dev/dri xg2g:v3.9.6 \
...
docker inspect --format='{{.Config.User}}' xg2g:v3.9.6
```

Test (no device):

```bash
docker run --rm xg2g:v3.9.6 \
...
```bash
docker run --rm --gpus all xg2g:v3.9.6 \
...
- name: Verify Deployment Contract
  run: |
    # FFmpeg wrapper
    docker run --rm xg2g:v3.9.6 sh -c '
