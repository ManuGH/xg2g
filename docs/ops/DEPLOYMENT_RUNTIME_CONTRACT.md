...
services:
  xg2g:
    image: ghcr.io/manugh/xg2g:v3.9.7
    container_name: xg2g
...
```bash
docker run --rm xg2g:v3.9.7 which ffmpeg
```
...
```bash
docker run --rm xg2g:v3.9.7 ffmpeg -version | head -1
```
...
```bash
docker run --rm xg2g:v3.9.7 sh -c 'echo $XG2G_FFMPEG_BIN'
```
...
```bash
docker run --rm -e FFMPEG_HOME=/nonexistent xg2g:v3.9.7 ffmpeg -version
```
...
```bash
docker run --rm --device /dev/dri:/dev/dri xg2g:v3.9.7 \
...
docker inspect --format='{{.Config.User}}' xg2g:v3.9.7
```

Test (no device):

```bash
docker run --rm xg2g:v3.9.7 \
...
```bash
docker run --rm --gpus all xg2g:v3.9.7 \
...
- name: Verify Deployment Contract
  run: |
    # FFmpeg wrapper
    docker run --rm xg2g:v3.9.7 sh -c '
