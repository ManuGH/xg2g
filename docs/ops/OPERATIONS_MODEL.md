...
- **Repo Truth**: The repo-side canonical deploy bundle lives under `deploy/`. For the systemd unit, the canonical source is `deploy/xg2g.service`; during the migration, `/srv/xg2g/docs/ops/xg2g.service` remains the deployed compatibility copy that the installed unit must strictly match.
...
- **Resource Pinning**: The container MUST use the pinned image `ghcr.io/manugh/xg2g:v3.4.3`.
