...
- **Repo Truth**: The repo-side canonical deploy bundle lives under `infra/systemd/`. For the systemd unit, the canonical source is `infra/systemd/xg2g.service`; the installed host unit must strictly match the deployed host copy under `/srv/xg2g/docs/ops/xg2g.service`.
...
- **Resource Pinning**: The container MUST use the pinned image `ghcr.io/manugh/xg2g:v3.10.0`.
