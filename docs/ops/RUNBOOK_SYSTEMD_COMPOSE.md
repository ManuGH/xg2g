...
**Common Issues**:
* **Start Fails immediately**: Check `systemctl status xg2g`. Ensure `XG2G_E2_HOST` (or `XG2G_OWI_BASE`) and `XG2G_API_TOKEN` are set in `/etc/xg2g/xg2g.env`. If both base URL vars are set, they must match exactly.
