...
- `apps/webui/src/components/V3Player.tsx`
- `apps/webui/src/features/resume/useResume.ts`
- `apps/webui/src/components/Config.tsx`
- `apps/webui/src/client-ts/**`
- `apps/webui/tests/contracts/**` (contract gates)
...
Findings from `rg "fetch\\(" apps/webui/src`:

1. `apps/webui/src/components/Config.tsx`
...
2. `apps/webui/src/components/V3Player.tsx`
...
- Legacy client decision helper was moved out of runtime source:
  - from `apps/webui/src/contracts/PolicyEngine.ts`
  - to `apps/webui/tests/contracts/helpers/PolicyEngine.ts` (test-only)
...
2. `npm --prefix apps/webui run gate:no-appledouble`
3. `npm --prefix apps/webui run gate:no-duration-guessing`
4. `npm --prefix apps/webui run gate:no-client-decision-engine`
5. `npm --prefix apps/webui run gate:no-ua-sniffing`
6. `npm --prefix apps/webui run gate:no-seek-resume-guessing`
7. `npm --prefix apps/webui run gate:no-raw-json-fetch`
8. `npm --prefix apps/webui run gate:no-raw-error-text`
9. `npm --prefix apps/webui run gate:mode-bridge`
10. `npm --prefix apps/webui run gate:seekable-contract`
