...
|-------|-------|----------|---------------------|
| **G204** | 2 | Subprocess Injection | **Marked Safe (#nosec)**. `ffmpeg` binary path is hardcoded/trusted from config; args are generated by strict internal logic. |
...
| **Errcheck** | 4 | Unchecked Return | **Fixed**. Explicitly ignored `Close()` errors in `defer` or added proper error handling (`SetRootContext`). |
