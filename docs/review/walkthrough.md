...
We performed a strict security audit to remove global `gosec` suppressions from the CI pipeline.

### Findings & Resolutions

| Issue | Count | Category | Resolution Strategy |
|-------|-------|----------|---------------------|
| **G204** | 3 | Subprocess Injection | **Marked Safe (#nosec)**. `ffmpeg` binary path is hardcoded/trusted from config; args are generated by strict internal logic. Local adapter specifically justified. |
| **G304** | 3 | File Inclusion | **Marked Safe (#nosec)**. File paths (`playlist.m3u8`, `.ts`, `deprecations.json`) are trusted from internal metadata or confined paths. |
| **Errcheck** | 4 | Unchecked Return | **Fixed**. Enabled `errcheck` linter. Fixed exposed failures (`defer Close`, `SetRootContext`). Production code is now `errcheck` clean. |

### Verification

Run the linter to confirm zero security issues:

```bash
golangci-lint run -E gosec ./...
```

The output should contain **no** `gosec` errors.
